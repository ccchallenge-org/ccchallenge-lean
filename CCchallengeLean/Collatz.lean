...
def C_Q (x : ℚ) : ℚ :=
  if x.num % 2 = 0 then x / 2 else 3 * x + 1
...
def oddIterateIndices_C_Q (x n : ℕ) : Finset ℕ :=
  Finset.range (n + 1) |>.filter (fun i : ℕ => Odd (C_Q^[i] x).num)

def numOddIterates_C_Q (x n : ℕ) : ℕ :=
  (oddIterateIndices_C_Q x n).card
...
def oddIterateIndices_T_Q (x n : ℕ) : Finset ℕ :=
  Finset.range (n + 1) |>.filter (fun i : ℕ => Odd (T_Q^[i] x).num)

def numOddIterates_T_Q (x n : ℕ) : ℕ :=
  (oddIterateIndices_T_Q x n).card
